...
const CONFIG = {
  ...
  characters: [
    { key:"boss", name:"老闆", color:"#2c14a3", bio:"怪談出版社的老闆，缺乏靈感而參加百物語",
      skill1:{ name:"怪談化身", desc:"擲1d8，結果×3", usesPerGame:3,
        action(ctx){ const r=rollDie(8); return { text:"老闆把故事扯得更黑、更長，他哪來這麼多奇怪故事？", rollValue:r, deltaToTotal:r*3, sprite:"skill1" }; } },
      skill2:{ name:"高壓催稿", desc:"自己擲1d20，所有人下一次擲骰=1d15", usesPerGame:2,
        action(ctx){ const r=rollDie(20) ;ctx.state.global.nextDieAll={sides:15,remaining:ctx.state.order.length}; return { text:"「這樣的進度不行啊，每個人多貢獻一點故事吧。」", rollValue:null, deltaToTotal:0, sprite:"skill2" }; } } },
    ...
    { key:"little", name:"妹妹", color:"#b466d4", bio:"老闆的小女兒，好奇心旺盛",
      skill1:{ name:"新手上路", desc:"強制擲出1或10", usesPerGame:3,
        action(ctx){ const r=Math.random()<0.5?1:10; return { text:"妹妹要嘛很短，要嘛直接爆衝。", rollValue:r, deltaToTotal:r, sprite:"skill1" }; } },
